"""Vercel function: GET /api/token?lang=xx[&pass=...]
A short-lived Cartesia token, so the key never reaches the browser.
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        code, body = server.token_payload(
            server.query_arg(self.path, "lang"),
            server.query_arg(self.path, "pass"),
        )
        server.write_json(self, code, body)
